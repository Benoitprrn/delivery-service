...
const SETUP_ERROR = 'Unable to obtain merchant test access token. Run scripts/seed-auth.sh and verify that local Supabase is running.'
...
export async function getMerchantAccessToken(): Promise<string> {
  ...
  try {
    response = await fetch(`${supabaseUrl}/auth/v1/token?grant_type=password`, {
      ...
      body: JSON.stringify({ email: 'merchant@test.fr', password: 'test1234' })
    })
  } catch {
